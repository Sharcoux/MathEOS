...
public final class Configuration {
    ...
    private static final String ADRESSE_SITE = "http://bomehc.fr";
    public static final String APPLICATION_ICON = "images/bomehc.png";

//    private static final String FICHIER_CONFIGURATION = System.getProperty("user.home")+Adresse.separatorChar+"Bomehc"+Adresse.separatorChar+"config.ini"; //adresse configuration initiale du logiciel
    ...
    static void setLangue(String langue) {
        if (!getLangue().equals(langue)) {
            ...
            if(!langue.equals(LANGUE_DEFAULT)) {//on modifie les données avec celles du nouveau fichier
                File adresse = new File(getDossierLangues()+ langue + "." + Adresse.EXTENSION_LANGUE);
                if(adresse.exists()) fLangue.fusionner(new FichierML(adresse));
            }
    ...
    static void setTheme(String theme) {
        if (!getTheme().equals(theme)) {
            ...
            if(!theme.equals(THEME_DEFAULT)) {//on modifie les données avec celles du nouveau fichier
                File adresse = new File(getDossierThemes()+ theme + "." + Adresse.EXTENSION_THEME);
                if(adresse.exists()) {
                    fSources.fusionner(new FichierML(adresse));
...
//    private static String getDonneeConfig(String s) {return userConfig.getContenu(s); }
    ...
    static {
        ...
        if(!installConfigFile.exists()) {
            ...
            installConfigFile = new Adresse(System.getProperty("user.home")+Adresse.separatorChar+"Bomehc"+Adresse.separatorChar+INSTALL_PARAMETERS_FILENAME);
            if(!installConfigFile.exists()) {
                ...
                properties.put("dossierUtilisateur", System.getProperty("user.home")+Adresse.separatorChar+"Bomehc"+Adresse.separatorChar);
                properties.put("dossierApp", System.getProperty("user.home")+Adresse.separatorChar+"Bomehc"+Adresse.separatorChar);
