...
import bomehc.IHM.ONGLET;
import bomehc.IHM.ONGLET_TEXTE;
import bomehc.clavier.Clavier;
import bomehc.clavier.ClavierCaractereSpeciaux;
import bomehc.clavier.ClavierNumerique;
import bomehc.elements.*;
import bomehc.graphic.fonctions.OngletFonctions;
import bomehc.graphic.geometrie.OngletGeometrie;
import bomehc.operations.OngletOperations;
import bomehc.sauvegarde.Data;
import bomehc.sauvegarde.DataCahier;
import bomehc.sauvegarde.DataFile;
import bomehc.sauvegarde.DataProfil;
import bomehc.proportionality.OngletProportionality;
import bomehc.texte.OngletCahierDEvaluation;
import bomehc.texte.OngletCahierDExercice;
import bomehc.texte.OngletCahierDeCours;
import bomehc.utils.boutons.ActionComplete;
import bomehc.utils.boutons.Bouton;
import bomehc.utils.dialogue.DialogueAbout;
...
import bomehc.utils.objets.Calculatrice;
import bomehc.utils.objets.Clock;
import bomehc.utils.objets.DataTexteDisplayer;
import bomehc.utils.objets.Loading;
...
//        TABLEAUX(ONGLET_TYPE.TP, "table",1,OngletTableaux.class),
    ...
    static void setTheme(String newTheme) {
        ...
        if (interfaceBomehc != null) { resetInterface(); }
    }
     ...
     */
    private static InterfaceComplete interfaceBomehc;
...
//            changerProfil(p);Déjà fait en fait
    ...
    static void startInterface() {
        interfaceBomehc = new InterfaceComplete(getThemeElement("laf"));
        System.out.println("interfaceBomehc created");
        initialize();
        System.out.println("interfaceBomehc initialized");
    }

    static void resetInterface() {
        //interfaceBomehc.sauvegarder(getCurrentProfile());
        interfaceBomehc.masque();
        interfaceBomehc.getFenetre().dispose();
        interfaceBomehc=null;
    ...
    public static JFrame getMainWindow() {return interfaceBomehc==null ? null : interfaceBomehc.getFenetre();}
    
    /**
     * Ferme la fenêtre principale de Bomehc, et donc l'application
     */
...
//    public static void addToSizeManager(Component c, int largeur, int hauteur) {
        ...
        //interfaceBomehc.getSizeManager().addToSizeManager(c, largeur, hauteur);
//    }
    public static void activeAction(PermissionManager.ACTION actionName, boolean active) {
        ...
        switch(actionName) {
            ...
            case FONCTIONS : 
                interfaceBomehc.setOngletTPEnabled(ONGLET.FONCTION.getIndex(), active);
            ...
            case CONSULTATION : 
                ...
                interfaceBomehc.setOngletCoursEnabled(ONGLET.COURS.getIndex(), active);
                interfaceBomehc.setOngletCoursEnabled(ONGLET.EXERCICE.getIndex(), active);
                break;
    ...
    public static void activeMode(boolean newMode) {
        interfaceBomehc.activeMode(newMode);
    }
    ...
    public static void setOngletActif(Onglet onglet) {
        interfaceBomehc.setOngletActif(onglet);
    }

    public static void setTaille(int taille) {
        interfaceBomehc.setTaille(taille);
    }
    ...
    public static class MenuOptions extends BarreMenu.Menu {
        public MenuOptions() {
            interfaceBomehc.getBarreMenu().super(BarreMenu.MENU_OPTION);
    ...
    public static boolean getMode() {
        return interfaceBomehc.getMode();
    }

    public static Onglet getOngletActif() {
        return interfaceBomehc.getOngletActif();
    }

    private static Onglet.OngletCours getOngletCoursActif() {
        return interfaceBomehc.getOngletCoursActif();
    }

    private static Onglet.OngletTP getOngletTPActif() {
        return interfaceBomehc.getOngletTPActif();
    }
        ...
        public void run() {
            switch(onglet.getType()) {
                case TEXTE:
                    ...
                    if(o!=null) { interfaceBomehc.addOngletCours(onglet.getNom(), o); }
                    break;
                case TP:
                    ...
                    if(p!=null) { interfaceBomehc.addOngletTP(onglet.getNom(), p); }
                    break;
            }
        }
    }
*/
    private static void initializeBarreMenu() {
        BarreMenu menu = interfaceBomehc.getBarreMenu();
...
//        menu.addElement(new ActionAide(), BarreMenu.AIDE);
        ...
        /*
         * /boutons menu.add(interfaceBomehc.boutonCouper); menu.add(interfaceBomehc.boutonCopier); menu.add(interfaceBomehc.boutonColler); menu.add(interfaceBomehc.boutonAnnuler); menu.add(interfaceBomehc.boutonRefaire);
         */
    ...
    private static void definirAdresseFichier(DataProfil profil) {
        ...
        fc.setFileFilter(new Adresse.BomehcFileFilter());
        String adresseParDefaut = profil.getNom()+ "_" + profil.getPrenom()+ "_" + profil.getClasse()+ "." + Adresse.EXTENSION_Bomehc;
        fc.setSelectedFile(new File(adresseParDefaut));
        int choix = fc.showSaveDialog(getMainWindow());
        if(choix==JFileChooser.APPROVE_OPTION) {
            File fichier = fc.getSelectedFile();
            if(!fichier.getPath().endsWith(Adresse.EXTENSION_Bomehc)) {fichier = new File(fichier.getAbsolutePath()+"."+Adresse.EXTENSION_Bomehc);}
...
//        actionSauvegarde.setEnabled(false);
    ...
    private static void chargement() {
        ...
        interfaceBomehc.getFenetre().setTitle(Configuration.getNomUtilisateur()+" - "+Configuration.getClasse());
    }
    ...
    private static final class ActionClavierCaracteresSpeciaux extends ActionClavier {
        ...
        protected Clavier creerClavier() {
            ...
            try{
                ...
                c.setLocation(Toolkit.getDefaultToolkit().getScreenSize().width - c.getWidth(), interfaceBomehc.getBarreMenu().getHeight() + interfaceBomehc.getBarreOutils().getHeight()+20);
            }
            return c;
        }
    }
...
//                JOptionPane.showMessageDialog(interfaceBomehc.getFenetre(), "L'utilisation de la calculatrice n'est pas autorisée", "Operation refusée", JOptionPane.WARNING_MESSAGE);
//                return;
//            }
            try {
...
//    public static void notifyEdit() {actionSauvegarde.setEnabled(true);}
    ...
    private static final class ActionSauvegarde extends ActionComplete {
        ...
        @Override
        public void actionPerformed(ActionEvent e) { //interfaceBomehc.sauvegarder(getCurrentProfile());
            sauvegarde();
        }
    }
    ...
    private static final class ActionDocx extends ActionComplete {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            int choix = fc.showSaveDialog(interfaceBomehc.getFenetre());
            if(choix==JFileChooser.APPROVE_OPTION) {
    ...
    public static void choixFichierExport(DataFile fileContent, String nomFichier) {
        //choix du fichier de destination
        String defaultName = Configuration.getDossierCourant()+File.separatorChar+(nomFichier!=null ? nomFichier : fileContent.getTitre())+"."+Adresse.EXTENSION_Bomehc_EXPORT_FILE;
        Adresse fichier;
        JFileChooser fc = new JFileChooser();
        fc.setDialogType(JFileChooser.SAVE_DIALOG);
        ...
        if(choix==JFileChooser.APPROVE_OPTION) {
            ...
            if(!fichier.getPath().endsWith(Adresse.EXTENSION_Bomehc_EXPORT_FILE)) {fichier = new Adresse(fichier.getAbsolutePath()+"."+Adresse.EXTENSION_Bomehc_EXPORT_FILE);}
            if(fichier.exists()) {
                DialogueBloquant.CHOICE decision = DialogueBloquant.dialogueBloquant("dialog file already exists", DialogueBloquant.MESSAGE_TYPE.WARNING, DialogueBloquant.OPTION.YES_NO, ImageManager.getIcone("overwright icon"));
    ...
    public static DataFile choixFichierImport() {
        ...
        fc.setDialogType(JFileChooser.OPEN_DIALOG);
        fc.addChoosableFileFilter(new Adresse.SingleFileFilter());
        int choix = fc.showOpenDialog(interfaceBomehc.getFenetre());
        if(choix==JFileChooser.APPROVE_OPTION) {
            Adresse fichier = new Adresse(fc.getSelectedFile());
            if(!fichier.getPath().endsWith(Adresse.EXTENSION_Bomehc_EXPORT_FILE) || !fichier.exists()) {return choixFichierImport();}
    ...
    private static void initialize() {
        ...
        interfaceBomehc.addPropertyChangeListener(new PropertyChangeListener() {
            @Override
        ...
        interfaceBomehc.getFenetre().addWindowListener(new WindowAdapter() {
            @Override
...
//                sauvegardeAuto();
    ...
    private static void initializeBarreBas() {
        BarreBas bas = interfaceBomehc.getBarreBas();
    ...
    private static void initializePartieTPCours() {
        ...
        for(ONGLET_TEXTE onglet : ONGLET_TEXTE.values()) {
            ...
            if(o!=null) {
                ...
                interfaceBomehc.addOngletCours(onglet.getNom(), o);
                c.time();
            }
...
//                case TABLEAUX:o = new OngletTableaux();break;
            ...
            if(o!=null) {
                onglet.toOnglet().setInstance(o);
                interfaceBomehc.addOngletTP(onglet.getNom(), o);
                c.time();
            }
            Loading.setValue(avancement+=8);
        }
