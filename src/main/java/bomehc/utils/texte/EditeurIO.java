...
import bomehc.texte.composants.JLabelImage;
...
import bomehc.texte.composants.JLabelNote;
...
public abstract class EditeurIO {
    ...
    /** Transforme le contenu en un objet DataTexte permettant de reconstruire les données créées avec Bomehc.
     ...
     */
    private static DataTexte genererDonneesBomehc(String htmlBrut, Map<String, Component> componentMap, ComposantTexte.SVG_RENDERING svgAllowed, boolean mathMLAllowed) {
...
//        donnees.contenuHTML = HTML3toHTML5Attributs(donnees.contenuHTML);
    ...
    /** lis les données Bomehc contenues dans le DataTexte et restitue le contenu de l'éditeur **/
    private static void insererDonneesBomehc(JMathTextPane jtp, DataTexte donnees, int offset, boolean copie) {
...
//        donnees.contenuHTML = HTML5toHTML3Attributs(donnees.contenuHTML);
        ...
        //capture des composants bomehc en vue de leur insertion
        Elements elements = doc.select("."+CLASS_MATHEOS);
        
        //HACK : on enregistre le innerHTML des spans bomehc car le HTMLDocument a tendance à
        ...
        for(Element e : doc.getAllElements()) {
            ...
            //Les objets Bomehc ont un traitement séparé qui est fait par ailleurs
...
//        applyFontSizeCorrection(jtp.getHTMLdoc().getDefaultRootElement(), jtp.getHTMLdoc(), 12);//EditeurKit.TAILLES_PT[0]);
    ...
    public static void importHtmlMathML(JMathTextPane jtp, String html, int offset) {
        ...
        //on supprime les balises bomehc qui pourraient gêner si elles étaient présentes
...
//            e.attr("id", MathTools.getId(math)+"");
        ...
        //on traite à présent le problème comme un import bomehc
    ...
    public static void charger(JMathTextPane jtp, DataTexte donnees, int offset) {
        ...
        insererDonneesBomehc(jtp, donnees, offset, false);
    ...
    public static void copy(JMathTextPane jtp, DataTexte donnees, int offset) {
        insererDonneesBomehc(jtp, donnees, offset, true);
    }
...
//        System.out.println(html);
    ...
    /**
     * Transforme le contenu du HTMLDocument en données Bomehc
     * Transforme le contenu du JMathTextPane en données Bomehc
    ...
    public static DataTexte getDonnees(String htmlBrut, Map<String, Component> componentMap) {
        return genererDonneesBomehc(htmlBrut, componentMap, ComposantTexte.SVG_RENDERING.SVG, true);
    }
    ...
    public static DataTexte getDonnees(JMathTextPane jtp, int startOffset, int length, ComposantTexte.SVG_RENDERING svgRendering, boolean mathMLAllowed) {
        ...
        DataTexte data = genererDonneesBomehc(html, jtp.getComponentMap(), svgRendering, mathMLAllowed);
        jtp.setCursor(previous==null ? CursorManager.getCursor(Cursor.TEXT_CURSOR) : previous);
        return data;
    }
...
//            result = XHTMLImporter.convert(doc.html().replace(Configuration.getURLDossierImagesTemp(), ""), Configuration.getURLDossierImagesTemp());
    ...
    public static void export2Docx(String htmlBrut, Map<String, Component> componentMap, File fichierDeDestination) {
        DataTexte data = genererDonneesBomehc(htmlBrut, componentMap, ComposantTexte.SVG_RENDERING.PNG, false);
    ...
    public static String export2html5(String htmlBrut, Map<String, Component> componentMap) {
        DataTexte data = genererDonneesBomehc(htmlBrut, componentMap, ComposantTexte.SVG_RENDERING.EMBED_SVG, false);
        String html5 = removeSpanTags(data.getContenuHTML());
    ...
    public static String export2htmlMathML(String htmlBrut, Map<String, Component> componentMap) {
        DataTexte data = genererDonneesBomehc(htmlBrut, componentMap, ComposantTexte.SVG_RENDERING.SVG, true);
